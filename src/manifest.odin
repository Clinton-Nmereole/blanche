...
manifest_save :: proc(db: ^DB) {

	// Access the manifest filename from the struct
	// (Ensure your DB struct has a pointer to manifest or stores the filename)
	filename := db.manifest.filename

	// 1. Write to a TEMP file first
	temp_filename := fmt.tprintf("%s.tmp", filename)
	...
	for level_idx := 0; level_idx < len(db.levels); level_idx += 1 {
		for file in db.levels[level_idx] {
			// open the file

			// Get the file level and write it to the manifest file
			level_buf: [8]byte
			endian.put_u64(level_buf[:], endian.Byte_Order.Little, u64(level_idx))
			os.write(manifest_file, level_buf[:])

			filesize_buf: [8]byte
			endian.put_u64(filesize_buf[:], endian.Byte_Order.Little, u64(file.meta.filesize))
			os.write(manifest_file, filesize_buf[:])
			...
			endian.put_u64(
				large_klen_buf[:],
				endian.Byte_Order.Little,
				u64(len(file.meta.lastkey)),
			)
			os.write(manifest_file, large_klen_buf[:])


			// Write the actual largest key
			os.write(manifest_file, file.meta.lastkey)
	...
	os.rename(temp_filename, db.manifest.filename)
...
manifest_load :: proc(filename: string) -> ^Manifest {
	...
	for {
		...
		// read the file size
		filesize_buf: [8]byte
		bytes_read, _ = os.read(file, filesize_buf[:])
		if bytes_read == 0 {break}
		if bytes_read < 8 {
			fmt.println("File is corrupted.")
			return nil
		}
		filesize, _ := endian.get_u64(filesize_buf[:], endian.Byte_Order.Little)
